...
$response = array();
 ...
 if (isset($_POST['from']) && isset($_POST['to'])) {
    ...
    $result = mysql_query("SELECT question.id as id, question.forUserId, content, time, categoryId, category.categoryName, userId, user.name as userName, isAnswered FROM question, user, category WHERE question.userId = user.id AND forUserId=-1 AND question.categoryId = category.id ORDER BY time DESC LIMIT ".$to) or die(mysql_error());
